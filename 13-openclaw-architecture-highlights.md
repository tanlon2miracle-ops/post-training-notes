...
## 附：真实请求案例——从 Discord 消息到 Git Push

下面是一个**真实发生的完整交互**，展示 OpenClaw 如何处理一条群聊消息并执行多步操作。
...
> "本地 markdown 然后上传到 https://github.com/tanlon2miracle-ops/post-training-notes"
...
│  Session 解析 → group:discord:1481255435108356269      │
│  信任级别：群聊（受限工具集）                            │
└───────────────────────────────────────────────────────┘
      │
      ▼
┌─ ③ 上下文组装 ────────────────────────────────────────┐
│  加载 session 历史（含之前的对话上下文）                 │
│  组合系统 prompt：AGENTS.md + SOUL.md + IDENTITY.md    │
│  注入 inbound metadata（sender/channel/timestamp）     │
│  记忆搜索：查找相关的历史笔记和操作经验                  │
└───────────────────────────────────────────────────────┘
      │
      ▼
...
│  ├─ 模型自动降级：调用 browser.open 打开 Substack      │
│  └─ browser.snapshot 获取文章完整内容（~15000字）       │
│                                                        │
│  Turn 2: 模型决定写本地文件                             │
│  ├─ 分析文章结构，用中文重写为技术笔记                   │
│  └─ 调用 write 创建 openclaw-architecture-notes.md     │
│                                                        │
│  Turn 3: 模型决定推送到 GitHub                          │
│  ├─ exec: git clone 目标仓库到临时目录                  │
│  ├─ exec: 复制文件覆盖旧版本                            │
│  ├─ exec: git add + git commit（含详细 commit message） │
...
┌─ ⑥ 响应交付 ──────────────────────────────────────────┐
│  Agent 生成简短状态回复                                  │
│  Discord 适配器格式化（无 markdown 表格，用 bullet list）│
│  通过 discord.js 发回 #笔记 频道                        │
...
- **工具降级**：`web_fetch` 被 Cloudflare 拦截后，Agent 自主切换到 `browser` 工具（Chromium 自动化），无需人工干预
- **编码问题处理**：仓库中旧文件是 GB2312 编码导致中文乱码，Agent 用 UTF-8 重写解决
- **多工具链式调用**：一条消息触发了 browser → write → exec(git) 等 8+ 次工具调用，全部自动编排
- **上下文连续性**：Agent 记住了前几条消息中提到的 Substack URL 和目标仓库，跨消息理解意图
- **安全边界**：虽然在群聊 session 中，但该频道被配置为允许 exec/browser 等工具（笔记频道，受信任成员）
